...
describe('Testes de integração da rota GET /tutorias/historico/aluno/:idAluno', () => {
  ...
  describe('Testes de validação da requisição', () => {
    test('Deve retornar 406 informando que o "Aluno" é inexistente', async () => {
      ...
      const response = await HELPER_TUTORIAS.historicoAluno(mongoid(), tokenAluno)
  ...
  describe('Mostrando o histórico de tutorias de um aluno com sucesso', () => {
    test('Deve retornar status 200 "ok" para um aluno que não possui tutorias no histórico', async () => {
      const DADOS_ALUNO = HELPER_ALUNOS.gerarDadosValidosParaCriarAluno()
    ...
    test('Deve retornar status 200 "ok" para um aluno que possui tutorias no histórico', async () => {
    ...
    test('Deve retornar status 200 "ok" para outro aluno que busca um histórico', async () => {
      const DADOS_TUTORIA = HELPER_TUTORIAS.gerarDadosValidosParaCriarTutoria()
      const DADOS_TUTORIA_ENCERRADA = HELPER_TUTORIAS.gerarDadosValidosParaEncerrarTutoria()
      ...
      const response = await HELPER_TUTORIAS.historicoAluno(aluno._id, aluno2.tokenAluno)
    ...
    test('Deve retornar status 200 "ok" para outro tutor que busca um histórico', async () => {
      ...
      const { aluno, tokenAluno } = await criarAluno(DADOS_ALUNO, disciplina)
      const tutor2 = await criarTutor(DADOS_TUTOR2, [disciplina3])
      ...
      const response = await HELPER_TUTORIAS.historicoAluno(aluno._id, tutor2.tokenTutor)
  ...
  describe('Testes de segurança', () => {
    test('Deve retornar 403 informando que o token é inexistente', async () => {
      ...
      const response = await HELPER_TUTORIAS.historicoAluno(aluno._id, '')
